...
final class cycle_and_dates_test extends advanced_testcase {
    ...
    /**
     * Tests date calculation from enrolment.
     */
    public function test_calculation_from_enrolment(): void {
        ...
        $enrolled = (int) $DB->get_field_sql($sql, [
            'courseid' => $course->id,
            'userid' => $user->id,
        ]);
        $config = (object) [
            'courseid' => $course->id,
            'triggertype' => 'enrolment',
            'intervaldays' => 365,
        ];
    ...
    /**
     * Tests date calculation from course completion.
     */
    ...
    /**
     * Tests that a fixed date uses the next yearly occurrence.
     */
    ...
    /**
     * Tests calculation from the last completed recertification cycle.
     */
    public function test_calculation_after_last_kopere_recert(): void {
        $this->resetAfterTest();
        $course = $this->getDataGenerator()->create_course();
        $user = $this->getDataGenerator()->create_user();
        $cycles = new manager();
